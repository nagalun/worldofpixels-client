...
OPT_DBG += -gsource-map
LD_DBG  += $(OPT_DBG) -s ASSERTIONS=2 -s STACK_OVERFLOW_CHECK=2 -s DEMANGLE_SUPPORT=1 --source-map-base "/"
OPT_DBG += -D DEBUG=1 -D DEBUG_BASE_URL='"http://localhost:13375"'
...
EM_CONF_LD += -s INCOMING_MODULE_JS_API=[]

EM_CONF_LD += -s ENVIRONMENT=web -s EXPORT_NAME=AppOWOP -s MODULARIZE=1
